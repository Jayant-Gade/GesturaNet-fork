...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (data) => { try { const cmd = JSON.parse(data.toString()); if (['enable','disable','camera_on','camera_off','set_smoothing','set_sensitivity'].includes(cmd.action)) sendToEngine(cmd); } catch (e) {} });
...
app.post('/receive-request', (req, res) => {
  Handshake.handleReceiveRequest(req, res);
});
